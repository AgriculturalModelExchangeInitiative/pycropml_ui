...
def simulation(datafile, vardata, params, init):
    ...
    t_gAImean = df[vardata.loc[vardata["Variables"]=="gAImean","Data columns"].iloc[0]].to_list()
    ...
    t_listGAITTWindowForPTQ = df[vardata.loc[vardata["Variables"]=="listGAITTWindowForPTQ","Data columns"].iloc[0]].to_list()
    ...
    for i in range(0,len(df.index)-1):
        ...
        deltaTT = t_deltaTT[i]
        pAR = t_pAR[i]
        listGAITTWindowForPTQ = t_listGAITTWindowForPTQ[i]
        ...
        currentZadokStage,hasZadokStageChanged,hasFlagLeafLiguleAppeared,listPARTTWindowForPTQ,hasLastPrimordiumAppeared,listTTShootWindowForPTQ,ptq,calendarMoments,canopyShootNumber,calendarDates,vernaprog,phyllochron,leafNumber,numberTillerCohort,tilleringProfile,averageShootNumberPerPlant,minFinalNumber,finalLeafNumber,phase,calendarCumuls= PhenologyComponent.model_phenology(phyllochron_t1,minFinalNumber_t1,aMXLFNO,currentdate,cumulTT,pNini,sDsa_sh,latitude,kl,calendarDates_t1,calendarMoments_t1,lincr,ldecr,pincr,ptq,pTQhf,B,areaSL,areaSS,lARmin,sowingDensity,lARmax,lNeff,rp,p,gAImean,pdecr,leafNumber_t1,maxTvern,dayLength,deltaTT,pAR,listPARTTWindowForPTQ_t1,listTTShootWindowForPTQ_t1,listGAITTWindowForPTQ,tTWindowForPTQ,vBEE,calendarCumuls_t1,isVernalizable,vernaprog_t1,minTvern,intTvern,vAI,maxDL,choosePhyllUse,minDL,hasLastPrimordiumAppeared_t1,phase_t1,pFLLAnth,dcd,dgf,degfm,ignoreGrainMaturation,pHEADANTH,finalLeafNumber_t1,sLDL,grainCumulTT,gAI,sowingDay,hasZadokStageChanged_t1,currentZadokStage,sowingDate,sDws,sDsa_nh,hasFlagLeafLiguleAppeared,der,hasFlagLeafLiguleAppeared_t1,tilleringProfile_t1,targetFertileShoot,dse,slopeTSFLN,intTSFLN,canopyShootNumber_t1)
