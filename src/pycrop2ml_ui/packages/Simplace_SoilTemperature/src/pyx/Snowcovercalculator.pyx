...
def model_snowcovercalculator(float cCarbonContent,
    ...
    ExtendedDescription: Calculates the snow cover, first soil layer temperature and biomass on ground factor depending on climate and soil conditions.
    ...
    #/TMEAN = Mean daily air temperature at 2 m (Ã‚Â°C)
    TMEAN=0.5 * (iTempMax + iTempMin)
    #/TAMPL = Amplitude of daily air temperature at 2 m (Ã‚Â°C)
    TAMPL=0.5 * (iTempMax - iTempMin)
    #/DST = Bare soil surface temperature (Ã‚Â°C)
    ...
    if SnowWaterContent < 1E-10:
        ...
        #/SoilSurfaceTemperature = Actual soil surface temperature (Ã‚Â°C), iSoilTempArray = Yesterday's temperature in the first layer (Ã‚Â°C)
    ...
    if SnowWaterContent == float(0) and not (iRAIN > float(0) and tiSoilTempArray < float(1)):
        ...
        #/SNPKT is the snow pack temperature (Ã‚Â°C)
...
def init_snowcovercalculator(float cCarbonContent,
    ...
    Albedo = 0.0
    #/taken from experimental fit from Thomas Gaiser, 2000, n=35, R2=0.97
